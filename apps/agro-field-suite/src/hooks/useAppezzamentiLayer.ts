...
const LAYER_SOURCE_PATH = "agrogea://appezzamenti";

/**
 * Proietta gli appezzamenti dell'azienda attiva (dominio agronomico, PGlite)
  ...
  useEffect(() => {
    ...
    if (existing) {
      ...
      layer = {
        ...
        style: {
          ...DEFAULT_LAYER_STYLE,
          // Base neutra (grigio): vale per gli appezzamenti senza coltura. Le
    ...
    if (!fittedOnce.current && appezzamenti.length > 0) {
      fittedOnce.current = true;
      const bounds = getLayerBounds(layer);
      if (bounds) mapControllerRef.current?.fitBounds(bounds);
    }
  }, [appezzamenti, campiCampagna, crops, mapControllerRef, styleEpoch]);
}
