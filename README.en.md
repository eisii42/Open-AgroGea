...
- 🚜 **Geofencing and Field Mode** — plan the job at the desk, then touch nothing in the tractor: on entering the parcel the **GPS recognises the field** and offers the planned task, a very-high-contrast screen shows speed, hectares and elapsed time while you work (with geotagged voice notes), and on completion the job **records itself in the logbook** with quantities recomputed on the area actually covered. Automatic warning when a treatment **re-entry interval** is still open on that field.
