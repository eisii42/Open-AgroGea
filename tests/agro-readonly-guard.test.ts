...
describe("isViewerReadOnly — decisione pura", () => {
  ...
  it("senza azienda attiva o senza email ⇒ NON read-only (Master/self-service)", () => {
    ...
    assert.equal(isViewerReadOnly({ memberships: m, aziendaAttivaId: null, email: "v@x.it" }), false);
    assert.equal(isViewerReadOnly({ memberships: m, aziendaAttivaId: COMPANY, email: null }), false);
  });
});
...
describe("store — guard centralizzato sulle mutazioni", () => {
  it("un VIEWER viene RESPINTO da una mutazione di dominio", async () => {
    ...
    await assert.rejects(
      () => useAgroStore.getState().aggiornaAzienda({ business_name: "Hack" }),
      /sola lettura/i,
    );
  });
  ...
  it("un OWNER NON viene bloccato dal guard read-only", async () => {
    ...
    await assert.rejects(
      () => useAgroStore.getState().aggiornaAzienda({ business_name: "Ok" }),
