...
export class MapController {
  ...
  /** Proiezione forzata per istanza; null = default globe (vedi enforceDefaultProjection). */
  private projectionOverride: maplibregl.ProjectionSpecification | null = null;
  ...
  init(
    ...
    options: {
      ...
      /**
       * Proiezione forzata per l'istanza (default: globe via DEFAULT_PROJECTION).
       * Se diversa da globe, il controllo globe viene nascosto: la vista resta
       * planimetrica fissa (es. agro-field-suite usa mercator per il disegno).
       */
    ...
    this.projectionOverride = options.projection ?? null;
    if (this.projectionOverride && this.projectionOverride.type !== "globe") {
      // Vista 2D fissa: niente toggle globo nell'interfaccia.
      this.controlVisibility.globe = false;
    }
    ...
    this.map = new maplibregl.Map({
      ...
      // AgroGea: l'export immagine del Print Composer dipende da questo flag.
  ...
  private enforceProjection(): void {
    if (!this.map) return;
    // AgroGea: un override per-istanza (es. mercator forzato in agro-field-suite)
    // ha priorità; altrimenti si onora la proiezione persistita nelle preferenze
    // di progetto (feature upstream), con fallback al default globe.
    const target: maplibregl.ProjectionSpecification =
      this.projectionOverride ?? {
        type: this.mapPreferences.projection ?? DEFAULT_PROJECTION.type,
      };
    try {
      if (this.map.getProjection()?.type === target.type) return;
      this.map.setProjection(target);
    } catch {
      this.map.once("idle", () => this.enforceProjection());
    }
  }
