...
export type { EditMode } from "maplibre-gl-geo-editor";
import {
...
// ---------------------------------------------------------------------------
// Programmatic draw / single-feature geometry edit (driven by the host app)
// ---------------------------------------------------------------------------

/** True when the GeoEditor control is mounted and ready to receive commands. */
export function isGeoEditorReady(): boolean {
  return pluginActive && geoEditorControl !== null;
}

/**
 * Start a draw mode programmatically (polygon / line / marker / …) instead of
 * relying on the toolbar buttons. Awaits Geoman's async init so the first call
 * right after the plugin activates does not no-op. Returns false when the
 * control is not (yet) available.
 */
export async function enableGeoEditorDrawMode(
  mode: DrawMode,
): Promise<boolean> {
...
/** Exit any active draw/edit mode (e.g. after a shape is captured). */
export function disableGeoEditorModes(): void {
  geoEditorControl?.disableAllModes();
}

/**
 * Turn on a specific EDIT mode (change / drag / rotate / scale / simplify / …)
 * on the features currently loaded in the editor, instead of relying on the
 * toolbar buttons. Used by hosts that render their own edit toolbar (e.g. the
 * agro field suite). Returns false when the control is not available.
 */
export function enableGeoEditorEditMode(mode: EditMode): boolean {
  if (!pluginActive || !geoEditorControl) return false;
  geoEditorControl.enableEditMode(mode);
  return true;
}

/**
 * Load a single existing feature into the editor and turn on vertex editing so
 * its boundary/track/position can be dragged. The feature becomes the editor's
 * sole sketch; the host reads the live geometry back from the Sketches store
 * layer (see the geo-editor capture bridge in @agrogea/core). Returns false
 * when the control is not available or the import fails.
 */
export async function startGeoEditorFeatureEdit(
  feature: Feature,
): Promise<boolean> {
  if (!pluginActive || !geoEditorControl) return false;
  await ensureGeomanReady();
  ...
  // Mirror the loaded feature into the store so the capture bridge has a
  // baseline to diff live vertex edits against, then enable editing. Points are
  // moved as a whole (drag); lines/polygons expose draggable vertices (change).
  syncSketchesToStore();
  // The Geoman display layers (feature + vertex handles) are shown only while
  // the Sketches store layer is visible (see setGeomanDisplayLayersVisibility).
  // The capture bridge normally keeps Sketches hidden, so make it visible for
  // the edit session — the bridge cooperates while a geomEdit session is active.
  {
    const store = useAppStore.getState();
    const layer = findSketchesLayer(store.layers);
    if (layer && !layer.visible) store.setLayerVisibility(layer.id, true);
  }
  const geomType = feature.geometry?.type;
  const isPoint = geomType === "Point" || geomType === "MultiPoint";
  const mode = isPoint ? "drag" : "change";
  // enableEditMode sets the control's activeEditMode, which the display logic
  // keys off to keep Geoman's editable layers visible.
  geoEditorControl.enableEditMode(mode);
  // Belt-and-suspenders: some Geoman builds don't engage global edit/drag from
  // the wrapper alone, leaving the vertices non-draggable. Force the global mode
  // directly, guarding against a double-toggle that would turn it back off.
  try {
    const gm = geomanInstance as unknown as {
    ...
    if (isPoint) {
      if (gm?.enableGlobalDragMode && !gm.globalDragModeEnabled?.()) {
        gm.enableGlobalDragMode();
      }
    } else if (gm?.enableGlobalEditMode && !gm.globalEditModeEnabled?.()) {
      gm.enableGlobalEditMode();
    }
  } catch {
    // Geoman API differences across versions must not break the session.
  }
  return true;
}

/**
 * Discard every sketch the editor holds and drop the Sketches store layer, so
 * nothing transient lingers after a create/edit is saved or cancelled.
 */
export async function clearGeoEditorSketches(): Promise<void> {
  geoEditorControl?.disableAllModes();
  await clearSketchesFromEditor();
  const store = useAppStore.getState();
  const sketches = findSketchesLayer(store.layers);
  if (sketches) {
    sketchesLayerId = null;
    store.removeLayer(sketches.id);
  }
}
